...
def ReadXmlLloydSodium():
    #载入数据处理模板
    file_object = open("Handle10822.tex","r")
    #将模板作为字符串存储在template文件中
    source = file_object.read().decode('utf-8', 'ignore')
    
    source = LloydSodium(source)

    return source

if __name__ == '__main__':
    nstr = ReadXmlLloydSodium()
    nfile = open('Handle10822finish.tex','w')
