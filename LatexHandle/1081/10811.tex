...
用逐差法计算条纹间距${\Delta}x$:
$${\Delta}x = \displaystyle\frac{\sum\limits_{i=1}^{10} x_{i+10}-x_i}{10{\times}10} = %% DELTA_X %%mm$$

计算波长$\lambda$:
$$\bar{b} = \display\frac{b_\text{正}+b_\text{反}}{2} = \frac{ (%% DATA_SMALL[0] %%-%% DATA_SMALL[1]%%)+(%% DATA_SMALL[2] %%-%% DATA_SMALL[3] %%)}{2} = %% B_SMALL %%$$

$$\bar{b'} = \display\frac{b_\text{正}+b_\text{反}}{2} = \frac{ (%% DATA_BIG[0] %%-%% DATA_BIG[1]%%)+(%% DATA_BIG[2] %%-%% DATA_BIG[3] %%)}{2} = %% B_BIG %%$$
...
$b$的不确定度：
$$ u(b) = \displaystyle\frac{%% B_SMALL %%{\times}0.025}{\sqrt{3}} =%% U_B1 %% $$
b'的不确定度：
$$ u(b) = \displaystyle\frac{%% B_BIG %%{\times}0.025}{\sqrt{3}} =%% U_B2 %% $$
S的不确定度：
